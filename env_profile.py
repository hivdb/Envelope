...
# Read from the excel file which was created by env_align.py
...
#print(subtype_pcnt.head(30))
#exit()

def create_profile(mutations_df, subtype = 'all'):
    if subtype != 'all':
        mutations_df = mutations_df[mutations_df['Subtype'] == subtype]
    mutations_df = mutations_df.iloc[:, 3:]

    frequencies_all_output_file = f'{GENE}_Frequencies_{subtype}.xlsx'
    frequencies_ge1pcnt_file = f'{GENE}_Frequencies_GE1pcnt_{subtype}.xlsx'
    frequencies_horizontal_profile_file = f'{GENE}_Profile_{subtype}.xlsx'
...
create_profile(mutations_df)
for subtype in SUBTYPES:
    create_profile(mutations_df, subtype)
